...
export const MultiYAxisChart: React.FC<ChartInfo> = (chartInfoProps: ChartInfo) => {    
    ...
    const { isLoading: isLogLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            updateChartData(transformData({ processlogsList: parsedData }), setChartData, chartInfoProps.hideExtraSeries);
    ...
    useEffect(() => {
        ...
        setChartOptions(updateChartOptions("white", "white", chartInfoProps, labelCount, !!chartInfoProps.hideExtraSeries));
    }, [chartInfoProps, chartData]);

    const handleExportToPdf = () => {
        handleExportToPDF(chartRef, chartOptions, chartInfoProps, !!chartInfoProps.hideExtraSeries);
    }
